...
bool CheckAnswer(FILE *MyAnswer, FILE *ReferenceAnswer); // 检查我的答案与参考答案是否相符的函数，答案完全正确返回true，否则返回false

// 文件路径，根据要匹配的数据集进行修改
const string packetPath = "./dataset/data/dataset1/packet1.txt";       // 数据集路径
const string rulePath = "./dataset/data/dataset1/rule1.txt";           // 规则集路径
const string myAnswerPath = "./MyAnswers/dataset1/Packet1Answer.txt";  // 运行结果路径
const string referenceAnswerPath = "./dataset/data/dataset1/ans1.txt"; // 参考答案路径，只有 dataset1 的 packet1 有参考答案，若对其他数据集进行匹配则该行无效

int main()
{
    // 打开或创建文件
    FILE *Packet = fopen(packetPath.c_str(), "r");
    FILE *Rule = fopen(rulePath.c_str(), "r");
    if (Packet != NULL && Rule != NULL)
        printf("Succeeded To Open Dataset!\nExecuting...\n");
    else
    ...
    FILE *Packet1AnswerFile = fopen(myAnswerPath.c_str(), "w");
    FILE *ReferenceAnswerFile = fopen(referenceAnswerPath.c_str(), "r");
    ...
    // 校对答案
    if (packetPath == "./dataset/data/dataset1/packet1.txt")
...
// 检查我的答案与参考答案是否相符的函数
bool CheckAnswer(FILE *MyAnswerFile, FILE *ReferenceAnswerFile)
{
    int i, flag = 1, MyAnswer, ReferenceAnswer, ErrorNum = 0;

    fclose(MyAnswerFile);
    MyAnswerFile = fopen(myAnswerPath.c_str(), "r");
